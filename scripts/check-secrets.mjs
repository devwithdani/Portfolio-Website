...
if (process.argv.includes('--history')) {
  for (const revision of git('rev-list', '--all')
    ...
    .split('\n')
    .filter(Boolean)) {
    for (const path of git('ls-tree', '-r', '--name-only', '-z', revision)
      .split('\0')
      .filter(Boolean)) {
...
console.log(
  failures
    ? `${failures} possible secret finding(s).`
    : 'No known secret patterns found. This is not a guarantee that no secrets exist.',
);
